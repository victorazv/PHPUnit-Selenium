...
class testeFormulario extends PHPUnit_Extensions_Selenium2TestCase
{
    ...
    public function testaFormulario()
    {
        ...
        $element->value('Marcela Tavares');

        // Preenche o email
        $this->byName('dados[email]')->value('ufba@ufba.com');
        ...
        $texto = $element->text();
        $this->assertEquals('Marcela Tavares', $texto);

        // Email
        $texto = $this->byId('resultadoEmail')->text();
        $this->assertEquals('ufba@ufba.comsssss', $texto);
